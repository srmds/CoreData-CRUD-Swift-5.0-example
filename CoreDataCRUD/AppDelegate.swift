...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        if var runCount:Int = defaults.integerForKey(Constants.UserDefaults.RunCount) {
            if(runCount == 0){
                print("First time app run, therefore creating importing event data...")
                if replicator.pull() {
                    print("Successfully imported event data.")
                }
            }
    ...
    lazy var applicationDocumentsDirectory: NSURL = {
        // The directory the application uses to store the Core Data store file. This code uses a directory named "com.io-pandacode.CoreDataCRUD" in the application's documents Application Support directory.
